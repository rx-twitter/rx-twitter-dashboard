...
- TwitterRX Dashboard は Astro SSR アプリ: `src/` 配下に `lib/`（ビジネスロジック）・
  `pages/api/`（API ルート）・`components/`（UI）の責務レイヤーを分離する（SRP 原則 V）
...
- [ ] CQXX [P] テスト容易性確認: DI 構造・外部 API アダプター層が設計通りか確認（原則 III）
- [ ] CQXX [P] メソッドサイズ確認:
      30行超過の関数を列挙し理由を記録、60行超過は修正（原則 IV）
- [ ] CQXX [P] SRP 確認: UI / API ルート / ビジネスロジック /
      DB の責務混在がないか確認（原則 V）
- [ ] CQXX `npm run lint && npm run format:check && npm run build` をすべてグリーンにする
- [ ] CQXX `npm run test` ですべてのテストがグリーンであることを確認
